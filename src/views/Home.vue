...
export default {
  ...
  created() {
    ...
    console.log(this.flatten([[[1, 2], 3], 4, 4, 3]));
  ...
  methods: {
    ...
    ...mapMutations(["delNum"]), //将`this.delNum`映射为this.$store.commit('delNum'),也可以将`this.delNum`映射为this.$store.commit('delNum'，amount)
    //actions进行异步操作
    ...mapActions(["increment1"]), //将`this.increment1`映射为this.$store.dispatch('increment1'),也可以将`this.increment1`映射为this.$store.dispatch('increment1'，amount)
    format(n) {
      let num = n.toString();
      let len = num.length;
      if (len <= 3) {
